...
def test1():
    banks = [0, 2, 7, 0]
    memory = Memory(banks)
    assert 5 == memory.realloate_till_loop()


def part1():
    memory = Memory(read())
    print(memory.realloate_till_loop())


class CuriousMemory(Memory):

    def cycle_size(self):
        redistributions = self.realloate_till_loop()
        return redistributions - self.states.index(self.banks)


def test2():
    banks = [0, 2, 7, 0]
    memory = CuriousMemory(banks)
    assert 4 == memory.cycle_size()


def part2():
    memory = CuriousMemory(read())
    print(memory.cycle_size())
